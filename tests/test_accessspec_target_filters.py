...
def test_start_access_spec_accepts_two_target_filters(monkeypatch):
    ...
    def fake_start_access(**kwargs):
        captured.update(kwargs)

    monkeypatch.setattr(reader.llrp, "startAccess", fake_start_access)
...
def test_start_access_spec_accepts_target_tuple(monkeypatch):
    reader = LLRPReaderClient("reader")
    captured = {}
    monkeypatch.setattr(
    ...
    assert captured["targetSpec"] == list(targets)


def test_start_access_spec_rejects_invalid_target_list():
    reader = LLRPReaderClient("reader")
    ...
    with pytest.raises(ValueError, match="at most two"):
        reader.start_access_spec(C1G2Read(), target_spec=[C1G2TargetTag(), object()])
