...
For the features documented in this repository, install the immutable `GuLuLuIt/sllurp` release tag `v3.1.1` or download its wheel from the [v3.1.1 GitHub Release](https://github.com/GuLuLuIt/sllurp/releases/tag/v3.1.1). The `sllurp` package on PyPI is a separately maintained distribution and is not an equivalent installation path for GuLuLuIT features.
...
```powershell
.\.venv\Scripts\python.exe -m pip install "git+https://github.com/GuLuLuIt/sllurp.git@v3.1.1"
...
The source tree currently identifies as version **3.1.1**. If `sllurp --version` reports a different version, confirm which environment and executable are active before troubleshooting reader behavior.
